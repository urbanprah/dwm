...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, iscentered, isfloating, isurgent, neverfocus, oldstate, isfullscreen, truefs;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
drawbar(Monitor *m)
{
	...
	if ((w = m->ww - sw - stw - x) > bh) {
		if (m->sel) {
			drw_setscheme(drw, scheme[m == selmon ? SchemeNorm : SchemeOff]);
...
focusstack(const Arg *arg)
{
	...
	if (!selmon->sel || (selmon->sel->isfullscreen && selmon->sel->truefs))
		return;
	if (arg->i > 0) {
...
movemouse(const Arg *arg)
{
	...
	if (!(c = selmon->sel))
		return;
	if (c->isfullscreen && c->truefs) /* no support resizing fullscreen windows by mouse */
		return;
	restack(selmon);
	ocx = c->x;
	ocy = c->y;
...
showhide(Client *c)
{
	...
	if (ISVISIBLE(c)) {
		...
		if ((!c->mon->lt[c->mon->sellt]->arrange || c->isfloating) && !c->isfullscreen && !c->truefs)
...
togglefloating(const Arg *arg)
{
	if (!selmon->sel)
		return;
	if (selmon->sel->isfullscreen && selmon->sel->truefs) /* no support for fullscreen windows */
		return;
