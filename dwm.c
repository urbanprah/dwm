...
enum { SchemeNorm, SchemeSel, SchemeWar }; /* color schemes */
...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, iscentered, isfloating, isurgent, neverfocus, oldstate, isfullscreen, issticky, ignorefakefs;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			char ch;
                        char* text;
                        char* ptr;
                        char clickable[1024], clean[1024];

			click = ClkStatusText;

                        int len = strlen(rawstext) + 1;
                        if (!(text = (char*) malloc(sizeof(char)*len)))
                            die("malloc");
                        ptr = text;
                        memcpy(text, rawstext, len);
                                ...
                                if (text[i] == '^' && !isCode) {
                                        ...
                                        while (text[++i] != '^') {
                                                if (text[i] == 'c' || text[i] == 'b')
                                                        i += 7;
                                                else if (text[i] == 'f') i++; /* FIXME: is this necessary */
                                                else if (text[i] == 'r') {
                                                        ++i;
                                                        for(int _i = 0; i < 3; _i++){
                                                            while (text[++i] != ',');
                        ...
                        copyvalidchars(clean, clickable);
                        x = selmon->ww - TEXTW(clean) - getsystraywidth() + lrpad;
	...
	if (cme->message_type == netatom[NetWMState]) {
		...
		|| cme->data.l[2] == netatom[NetWMFullscreen])
			setfullscreen(c, (cme->data.l[0] == 1 /* _NET_WM_STATE_ADD    */
				|| (cme->data.l[0] == 2 /* _NET_WM_STATE_TOGGLE */ && !c->isfullscreen && !c->ignorefakefs)));
...
configurenotify(XEvent *e)
{
	...
	if (ev->window == root) {
		...
		if (updategeom() || dirty) {
			...
			for (m = mons; m; m = m->next) {
				for (c = m->clients; c; c = c->next)
					if (c->ignorefakefs && c->isfullscreen)
...
movemouse(const Arg *arg)
{
	...
	if (!(c = selmon->sel))
		return;
	if (c->isfullscreen && c->ignorefakefs) /* no support resizing fullscreen windows by mouse */
		return;
	restack(selmon);
	ocx = c->x;
	ocy = c->y;
...
resizeclient(Client *c, int x, int y, int w, int h)
{
	...
	if (((nexttiled(c->mon->clients) == c && !nexttiled(c->next))
	    || &monocle == c->mon->lt[c->mon->sellt]->arrange)
            && !c->isfloating && (!c->ignorefakefs || (c->ignorefakefs && !c->isfullscreen))) {
		c->w = wc.width += c->bw * 2;
...
showhide(Client *c)
{
	...
	if (ISVISIBLE(c)) {
		...
		if ((!c->mon->lt[c->mon->sellt]->arrange || c->isfloating) && !c->isfullscreen && !c->ignorefakefs)
...
togglefloating(const Arg *arg)
{
	if (!selmon->sel)
		return;
	if (selmon->sel->isfullscreen && selmon->sel->ignorefakefs) /* no support for fullscreen windows */
		return;
